...
def main(argv):
    ...
    jmx = subprocess.Popen(
        ["java", "-enableassertions", "-enablesystemassertions",  # safe++
         ...
         "accumulo", "",                     # All HBase / hadoop metrics.
    ...
    try:
        ...
        while True:
            ...
            if mbean_domain not in ("accumulo.server.metrics", "java.lang"):
                print >>sys.stderr, ("Unexpected mbean domain = %r on line %r"
                                     % (mbean_domain, line))
                continue
            mbean_properties = dict(prop.split("=", 1)
                                    for prop in mbean_properties.split(","))
            if mbean_domain == "accumulo.server.metrics":
              jmx_service = mbean_properties.get("service", "TabletServer")
            ...
            sys.stdout.write("accumulo.%s %d %s%s\n"
